...
function createFullPostBlock(post) {
    ...
    // Title and date block - cascading
    html += `<div class="blog-block ${getPos(1)}">`;
    ...
    // Check if this is a Q&A format post
    if (post.format === 'qa') {
        // Q&A format: content on left, image on right (if present)
        html += '<div class="qa-layout">';

        // Left side: Q&A content
        html += '<div class="qa-left">';
        ...
        // Regular format: cascading paragraphs with images
        const paragraphs = post.body.split('\n\n');
        let imgCounter = 2;

        paragraphs.forEach((para, index) => {
            // Add paragraph
            html += `<div class="blog-block ${getPos(index + 2)}">`;
            html += '<div class="blog-post-content">';
            html += `<div class="blog-post-body">${textToHTML(para)}</div>`;
            html += '</div></div>';

            // Add placeholder image after every 1-2 paragraphs
            if (index < paragraphs.length - 1 && (index % 2 === 0)) {
                html += `<div class="blog-block ${getPos(imgCounter + 3)}">`;
                if (post.image && index === 0) {
                    html += `<img src="${post.image}" alt="${post.title}" class="blog-post-image">`;
                } else {
                    html += `<div class="blog-placeholder-img ${getShape(imgCounter)}"></div>`;
                }
                html += '</div>';
                imgCounter++;
            }
        ...
        // End with another placeholder
        html += `<div class="blog-block ${getPos(imgCounter + 4)}">`;
        html += `<div class="blog-placeholder-img ${getShape(imgCounter + 1)}"></div>`;
        html += '</div>';
    }
